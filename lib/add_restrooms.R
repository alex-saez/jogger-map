
# Code for assigning restrooms to Manhattan street segments.

library(data.table)
library(dplyr)
library(mgcv)

restrooms = fread('../data/publictoilet.csv')
blocks = fread('../output/blocks_Manhattan.csv')
node_list = fread('../output/node_list.csv')
manhattan_outline = fread('../data/Boundary.csv')
source('../lib/assign_to_segment.R')

restrooms = restrooms %>%
              select(LNG, LAT) %>%
              mutate(LNG = as.numeric(LNG), LAT = as.numeric(LAT)) %>%
              filter( !is.na(LNG) & !is.na(LAT)) %>%
              as.matrix()
manhattan_outline = as.matrix(manhattan_outline)
restrooms = restrooms[in.out(manhattan_outline, restrooms),] # select manhattan restrooms only
node_list = filter(node_list, segment_id %in% blocks$PHYSICALID) # manhattan nodes only


# assign a segment ID to each tree:
segments_restrooms = apply(restrooms, 1, FUN=assign_to_segment, node_list)

# count restrooms per segment:
restrooms_per_seg = data.frame(table(segments_restrooms))

restrooms_per_seg = restrooms_per_seg %>%
                    select(seg_ID = segments_restrooms, n_restrooms = Freq) %>%
                    mutate(seg_ID = as.integer(as.character(seg_ID)))

blocks = left_join(blocks, restrooms_per_seg, by=c('PHYSICALID'='seg_ID'))

blocks$n_restrooms[is.na(blocks$n_restrooms)] = 0 # replace NAs with 0s


write.csv(blocks, file = '../output/blocks_Manhattan.csv', row.names = FALSE)


# for checking that it works: restrooms in red, assigned segments in blue
library(leaflet)
segs = which(blocks$n_restrooms!=0)
m <- leaflet() %>% addTiles()
for(i in segs){
  m = addPolylines(m, lng = c(blocks$start_lon[i], blocks$end_lon[i]),
                   lat = c(blocks$start_lat[i], blocks$end_lat[i]),
                   opacity = 1
  )
}
for(i in 1:nrow(restrooms)){
  m = addCircles(m, lng = c(restrooms[i,1]), lat = c(restrooms[i,2]), color = 'red', opacity = 1)
}
m

